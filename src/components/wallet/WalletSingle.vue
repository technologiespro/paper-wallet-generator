    ...
    <div class="row" v-if="accounts[0]">
      <div class="col-md-6 text-center border-right">
        ...
        <VueQrcode :options="{size:150, foreground: '#232D3D',level: 'H'}" :value="accounts[0].address"
                   id="qrPub"/>
        <p class="bg-success text-white p-1"><CopyClipboard :value="accounts[0].address"/> {{ accounts[0].address }}</p>
        <hr/>
      ...
      <div class="col-md-6 text-center">
        ...
        <VueQrcode :options="{size:150, foreground: '#232D3D',level: 'H'}" :value="accounts[0].wif"
                   id="qrPriv"/>
        <p class="bg-danger text-white p-1"><span class=""><CopyClipboard :value="accounts[0].privateKey"/> {{ accounts[0].privateKey }}</span></p>
        <hr/>
        <p>
